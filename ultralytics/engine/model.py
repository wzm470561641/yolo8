...
class Model(nn.Module):
    ...
    def _new(self, cfg: str, task=None, model=None, verbose=False) -> None:
        ...
        if (
            cfg_dict.get("ch", 3) == 4
            and DEFAULT_CFG_DICT.get("rgba", False) == False
            or cfg_dict.get("ch", 3) != 4
            and DEFAULT_CFG_DICT.get("rgba", False) == True
        ):
            raise ValueError(
                f"If the input image is in RGBA format, the parameter ch and rgba should be 4 and True respectively"
            )
        self.cfg = cfg
